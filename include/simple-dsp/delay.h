...
namespace simpledsp {
  ...
  struct DelayBasics<DelayAccessType::READ_THEN_WRITE> {
    ...
    sdsp_nodiscard static constexpr bool isValidDelay(
      ...
      return is_within(delaySamples,
              getMinimumDelay<SizeType>(),
    ...
    sdsp_nodiscard static constexpr SizeType getValidDelay(
      ...
      throw std::invalid_argument(
              "Basics::getDelta: delaySamples must lie between 1 and the buffer size");
    }
  ...
  struct DelayBasics<DelayAccessType::WRITE_THEN_READ> {
    ...
    sdsp_nodiscard static constexpr SizeType getValidDelay(
      ...
      throw std::invalid_argument(
              "Basics::getDelta: delaySamples must lie between 0 and the buffer size - 1");
    }
  ...
  private:
    ...
    SizeType setAndGetWriteDelay(SizeType readIndex, SizeType writeIndex, SizeType delay) {
      ...
      container_.at(writeIndex) = metric_.add(container_.at(readIndex),
              Basics::getValidDelay(metric_, delay));
      return result;
    }

    SizeType setAndGetReadDelay(SizeType readIndex,
            SizeType writeIndex,
            SizeType delay,
            bool check) {
