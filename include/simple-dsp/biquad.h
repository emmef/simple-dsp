...
namespace simpledsp {
  ...
  struct Biquad {
    ...
    static constexpr float CENTER_RELATIVE_MINIMUM = std::numeric_limits<float>::epsilon();
    static constexpr float CENTER_RELATIVE_MAXIMUM = 0.5 - std::numeric_limits<float>::epsilon();
    static constexpr float GAIN_MINIMUM = 0.01;
    static constexpr float GAIN_MAXIMUM = 100;
    static constexpr const char* TYPES_WITH_GAIN = "|PARAMETRIC|SHELVE_HIGH|SHELVE_LOW|";
    static constexpr const char* TYPES_WITH_SLOPE = "|SHELVE_HIGH|SHELVE_LOW|";
    static constexpr const char* TYPES_WITH_BANDWIDTH = "|PARAMETRIC|PASS_BAND|PASS_LOW|PASS_HIGH|";

    sdsp_nodiscard static const char * getTypeName(Type type) { return typeName(type); }

    sdsp_nodiscard static Biquad relative(
            Type type, IIRCalculationMethod method = IIR_CALCULATION_METHOD_DEFAULT) {
      return Biquad(type, 1.0, method);
    }

    sdsp_nodiscard static Biquad forSampleRate(
    ...
    Biquad &setCenter(float newCenter) {
      center = std::clamp(newCenter,
              CENTER_RELATIVE_MINIMUM * rate,
  ...
  private:
    ...
    static void checkAccess(const char *whatToAccess, Type type, const char * allowedTypes) {
    ...
    explicit Biquad(Type tp, float sampleRate, IIRCalculationMethod m) :
            type(verifyType(tp)), rate(sampleRate),
            center(rate / 4.0f), gain(1.0), widthOrSlope(1.0), method(m) {
    }
    ...
    void setCoefficients(IIRCoefficientsSetter &setter, const BiQuadCoefficients bqc) {
      setter.setC(0, bqc.C0);
      setter.setC(1, bqc.C1);
      setter.setC(2, bqc.C2);
      setter.setD(0, 0);
      setter.setD(1, method == IIRCalculationMethod::POSITIVE_Y ? bqc.D1 : -bqc.D1);
      setter.setD(2, method == IIRCalculationMethod::POSITIVE_Y ? bqc.D2 : -bqc.D2);
    }
    ...
    void generateParametric(IIRCoefficientsSetter& setter) {
      ...
      double J = sqrt(gain);
      double g = sw
              * sinhf(LN_2_2 * widthOrSlope * omega / sw);
