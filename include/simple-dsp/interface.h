...
namespace simpledsp {
  ...
  class Interface {
    size_t inputs_;
    size_t outputs_;
    SampleRate<freq> sampleRate_;
    bool realTime_;
    size_t bufferSize_;
#if defined(SIMPLE_DSP_INTERFACE_SIZELIMIT) && SIMPLE_DSP_INTERFACE_SIZELIMIT > 0
  ...
  public:
    ...
    const SampleRate<freq> sampleRate() const noexcept { SampleRate r(sampleRate_); return r; }
    bool realTime() const noexcept { return realTime_; }
    size_t bufferSize() const noexcept { return bufferSize_; }

    static Interface of(
            size_t inputs, size_t outputs, SampleRate<freq> sampleRate, bool realTime,
            size_t bufferSize) {
    ...
    Interface withInputs(size_t inputs) const {
      ...
      throw std::invalid_argument("Interface::withInputs(inputs): Invalid number of inputs or "
    ...
    Interface withOutputs(size_t outputs) const {
      ...
      throw std::invalid_argument("Interface::withOutputs(outputs): Invalid number of outputs or "
                                  "invalid in combination with other parameters.");
    }
    ...
    Interface withBufferSize(size_t bufferSize) const {
      ...
      throw std::invalid_argument("Interface::withBufferSize(bufferSize): Invalid buffer size or "
                                  "invalid in combination with other parameters.");
    }

    Interface withSampleRate(SampleRate<freq> sampleRate) const noexcept {
