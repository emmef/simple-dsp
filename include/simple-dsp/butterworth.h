...
namespace simpledsp {
    ...
    struct Butterworth {
      ...
      static constexpr float FREQUENCY_RELATIVE_MINIMUM = std::numeric_limits<float>::epsilon();
      static constexpr float FREQUENCY__RELATIVE_MAXIMUM = 0.5 -
      ...
      Butterworth &setCenter(float newCenter) {
        frequency = std::clamp(newCenter,
                FREQUENCY_RELATIVE_MINIMUM * rate,
    ...
    private:
      ...
      type(verifyType(tp)), rate(sampleRate),
              frequency(rate / 4.0f), order(1), method(m) {
      }
      ...
      static void getDCoefficients(
              unsigned order, double relativeFrequency, IIRCalculationMethod method,
              IIRCoefficientsSetter &d_coefficients)
      {
        ...
        for (unsigned i = 0; i < order; ++i) {
          for (int j = i; j > 0; --j) {
            dcof[2 * j] += binomials[2 * i] * dcof[2 * (j - 1)] - binomials[2 * i + 1] * dcof[2 * (j - 1) + 1];
            dcof[2 * j + 1] +=
                    binomials[2 * i] * dcof[2 * (j - 1) + 1] + binomials[2 * i + 1] * dcof[2 * (j - 1)];
          }
      ...
      static double getLowPassScalingFactor(unsigned order, double relativeFrequency)
      {
      ...
      static double getHighPassScalingFactor(unsigned order, double relativeFrequency)
      {
        unsigned k; // loop variables
        double omega; // M_PI * relativeFrequency
        double fomega; // function of omega
