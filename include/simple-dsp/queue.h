...
namespace simpledsp {
  ...
  namespace queue_position {
    ...
    /**
     * Queue positions manage the read and write position of a queue. All positions must
     * implement the methods in this Traits class.
     *
     * The implementations simpledsp::queue_position::Unsafe, simpledsp::queue_position::Atomic
     * and simpledsp::queue_position::Guarded are all lock-free.
     *
     * @TODO: Actually use this class to test if the position implementations have these methods.
     */
    template <class Implementation>
    struct PositionTraits {
      /**
       * Enters the context where positions are used and reports if that was successful. If it
       ...
       */
      sdsp_nodiscard sdsp_force_inline bool enterAndLoad(size_t& rd, size_t& wr) noexcept {
      ...
      sdsp_force_inline void storeRead(size_t newReadPosition) noexcept {
        return reinterpret_cast<Implementation *>(this)->storeReadTrait(newReadPosition);
      }
      ...
      sdsp_force_inline void storeWrite(size_t newWritePosition) noexcept {
        return reinterpret_cast<Implementation *>(this)->storeWriteTrait(newWritePosition);
      }

      /**
       * Leaves the context in which manipulation to positions (and data) happens.
       *
       * If this method is not invoked, there is no guarantee that other threads will see any
       * changes in the queue position or content. In case of a guarded position, not invoking
       * this method can leave the queue inaccessible forever.
       */
    ...
    /**
     * A position implementation that offers no guarantees at all concerning memory visibility or
     * queue position consistency.
     *
     * Queues that use this implementation are usefull when wrapped in a queue that does full
    ...
    /**
     * A position implementation that offers memory visibility but not necessarily consistency.
     * This implementation can be used for queues where all puts happen in one thread and all
     * gets happen in one thread. If that cannot be guaranteed, use the Guarded variant.
     */
  ...
  namespace queue_data {
    /**
     * Contains the values for a queue. The number of values uf the capacity() and the
     * maximum value that can be passed to the #operator[](size_t) operators is that capacity.
     * This means that for a straightforward implementation, the allocated size should be the
     * capacity plus one.
    ...
    public:
      /**
       * Returns the capacity of the queue when using this data. This should be one less than the
       * allocated size.
    ...
    class DefaultData : public DataTraits<Value, DefaultData<Value>> {
      ...
      static size_t validCapacity(size_t capacity) {
        ...
        throw std::invalid_argument("BaseQueue<V>: Given capacity zero or too large for value type.");
      }
   ...
   *
   * @tparam Value The type of values to store, that must be trivial to construct, copy and assign.
   * @tparam Position The type that managers the read and write positions and provides specified
   * visibility and consistency guarantees.
   * @tparam Data The type of storage to use, see simpledsk::queue_data::DataTraits
  ...
  class BaseQueue {
    static_assert(
            std::is_trivially_copy_assignable<Value>::value,
            "Value type must be trivially copy-assignable");

    static_assert(
            std::is_base_of<queue_position::PositionTraits<Position>, Position>::value,
  ...
  public:
    ...
    /**
     * Creates a queue whose storage is determined by the arguments given to the underlying
     * storage type.
     * @see the data
     * @param capacity The maximum number of elements in the queue.
     */
    template <typename... DataParameters>
    BaseQueue(DataParameters... parameters) :
            data_(parameters...) {

    }

    /**
     * Returns the capacity of the queue: the maximum number of elements it can hold.
     * @return The capacity of the queue.
     */
    sdsp_nodiscard size_t capacity() const noexcept {
     ...
     *
     * If the position type used does not guarantee consistence and no other measures are taken
     * by the caller, the returned value can be wrong.
     *
    ...
    /**
     * Returns the queue state, that indicates if putting or getting elements will work.
     *
     ...
     *
     * This method can be dangerous to use with a position implementation that does not guarantee
     * position consistency and where the caller does not provide such a guarantee.
  ...
  private:
    ...
    sdsp_nodiscard bool isFull(size_t rd, size_t wr, size_t &next) const noexcept {
      next = nextValue(wr);
      return next == rd;
    }
  ...
  /**
   * A queue that ensures visibility but does not protect against put-put or get-get data races.
