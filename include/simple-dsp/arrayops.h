...
namespace simpledsp::arrayops {
  ...
  template<typename T>
  sdsp_force_inline static void
  inline_add_to_with_factor(T* __restrict destination,
  ...
  static void add_to_with_factor(T* destination, const T* source, T factor, size_t size) {
    inline_add_to_with_factor(assumeAligned<ALIGNMENT>(destination),
            assumeAligned<ALIGNMENT>(source),
            factor,
            size);
  }
  ...
  static void add_to_with_factor(AlignedData<T, ALIGNMENT, C>& destination,
          const AlignedData<T, ALIGNMENT, D>& source,
          T factor) {
    inline_add_to_with_factor(destination.ptr()(),
            source.ptr()(),
            ::std::min(destination.size(), factor, source.size()));
  }

  template<typename T, size_t ALIGNMENT, class C, class D>
  static void add_to_with_factor_frames_up(AlignedData<T, ALIGNMENT, C>& destination,
          const AlignedData<T, ALIGNMENT, D>& source,
          T factor,
          size_t framesUp) {
  ...
  static void subtract_from(AlignedData<T, ALIGNMENT, C>& destination,
          const AlignedData<T, ALIGNMENT, D>& source) {
    inline_subtract_from(destination.ptr()(),
            source.ptr()(),
            ::std::min(destination.size(), source.size()));
  }
  ...
  template<typename T, size_t SIZE, size_t ALIGNMENT>
  static void subtract_from_with_factor(AlignedArray<T, SIZE, ALIGNMENT>& destination,
          const AlignedArray<T, SIZE, ALIGNMENT>& source,
          T factor) {
  ...
  template<typename T, size_t ALIGNMENT, class C, class D>
  static void subtract_from_with_factor(AlignedData<T, ALIGNMENT, C>& destination,
          const AlignedData<T, ALIGNMENT, D>& source,
          T factor) {
    inline_subtract_from_with_factor(destination.ptr()(),
            source.ptr()(),
            ::std::min(destination.size(), factor, source.size()));
  }

  template<typename T, size_t ALIGNMENT, class C, class D>
  static void subtract_from_with_factor_frames_up(AlignedData<T, ALIGNMENT, C>& destination,
          const AlignedData<T, ALIGNMENT, D>& source,
          T factor,
          size_t framesUp) {
    inline_subtract_from_with_factor(
            ...
            ::std::min(
                    ::std::max(0,
                            static_cast<ssize_t>(
                                    destination.size()) - framesUp * AlignedData<T,
                                                                                 ALIGNMENT,
                                                                                 C>::Metric::alignmentElements),
                    source.size()));
  }
