...
namespace simpledsp::arrayops {
    ...
    inline_multiply_with(T *v1, T factor, size_t size) {
        for (size_t i = 0; i < size; ++i) {
            v1[i] *= factor;
        }
    }
    ...
    template <typename T>
    sdsp_force_inline static void
    inline_multiply_with_range(T *v1, T from, T to, size_t size) {
        T factor = from;
    ...
    template <typename T>
    sdsp_force_inline static void
    inline_add_to(T *__restrict destination, const T *__restrict source, size_t size) {
        for (size_t i = 0; i < size; ++i) {
    ...
    template <typename T>
    sdsp_force_inline static void
    inline_add_to(T *destination, T delta, size_t size) {
        for (size_t i = 0; i < size; ++i) {
            destination[i] += delta;
        }
    }
    ...
    template <typename T>
    sdsp_force_inline static void
    inline_add_to_with_factor(T *__restrict destination, const T *__restrict source, T factor, size_t size) {
        for (size_t i = 0; i < size; ++i) {
    ...
    sdsp_force_inline static void
    inline_subtract_from(T *__restrict destination, const T *__restrict source, size_t size) {
        for (size_t i = 0; i < size; ++i) {
            destination[i] -= source[i];
        }
    }
    ...
    template <typename T>
    sdsp_force_inline static void
    inline_subtract_from(T *destination, T delta, size_t size) {
        for (size_t i = 0; i < size; ++i) {
            destination[i] -= delta;
        }
    }
    ...
    template <typename T>
    sdsp_force_inline static void
    inline_subtract_from_with_factor(T *__restrict destination, const T *__restrict source, T factor, size_t size) {
        for (size_t i = 0; i < size; ++i) {
            destination[i] -= source[i] * factor;
        }
    }
    ...
    /*
     * Calculate the inner product of an array with itself (the sum of the squares of all elements).
     */
    ...
    [[nodiscard]] static T inner_product(const AlignedData<T, ALIGNMENT, C> &array1, const AlignedData<T, ALIGNMENT, D> &array2) {
        return inline_inner_product(array1.ptr(), array2.ptr(), ::std::min(array1.size(), array2.size()));
    }

    template <typename T, size_t ALIGNMENT, class C, class D>
    [[nodiscard]] static T operator * (const AlignedData<T, ALIGNMENT, C> &array1, const AlignedData<T, ALIGNMENT, D> &array2) {
        return inline_inner_product(array1.ptr(), array2.ptr(), ::std::min(array1.size(), array2.size()));
    }

    template <typename T, size_t SIZE, size_t ALIGNMENT>
    [[nodiscard]] static T inner_product(const AlignedArray<T, SIZE, ALIGNMENT> &array1, const AlignedArray<T, SIZE, ALIGNMENT> &array2) {
        return inline_inner_product(array1.ptr(), array2.ptr(), SIZE);
    }

    template <typename T, size_t SIZE, size_t ALIGNMENT>
    [[nodiscard]] static T operator * (const AlignedArray<T, SIZE, ALIGNMENT> &array1, const AlignedArray<T, SIZE, ALIGNMENT> &array2) {
        return inline_inner_product(array1.ptr(), array2.ptr(), SIZE);
    }
    ...
    template <typename T, size_t SIZE, size_t ALIGNMENT>
    static void multiply_with_range(AlignedArray<T, SIZE, ALIGNMENT> &array, T from, T to) {
        inline_multiply_with_range(array.ptr()(), from, to, SIZE);
    }

    template <typename T, size_t ALIGNMENT, class C>
    static void multiply_with_range(AlignedData<T, ALIGNMENT, C> &array, T from, T to) {
    ...
    static void multiply_with_range_accurate(T * array, T from, T to, size_t size) {
        inline_multiply_with_range_accurate(assumeAligned<ALIGNMENT>(array), from, to, size);
    }

    template <typename T, size_t SIZE, size_t ALIGNMENT>
    static void multiply_with_range_accurate(AlignedArray<T, SIZE, ALIGNMENT> &array, T from, T to) {
        inline_multiply_with_range_accurate(array.ptr()(), from, to, SIZE);
    }

    template <typename T, size_t ALIGNMENT, class C>
    static void multiply_with_range_accurate(AlignedData<T, ALIGNMENT, C> &array, T from, T to) {
    ...
    static void add_to(T * destination, const T * source, size_t size) {
        inline_add_to(assumeAligned<ALIGNMENT>(destination), assumeAligned<ALIGNMENT>(source), size);
    }

    template <typename T, size_t SIZE, size_t ALIGNMENT>
    static void add_to(AlignedArray<T, SIZE, ALIGNMENT> &destination, const AlignedArray<T, SIZE, ALIGNMENT> &source) {
        inline_add_to(destination.ptr()(), source.ptr()(), SIZE);
    }

    template <typename T, size_t SIZE, size_t ALIGNMENT>
    static void operator += (AlignedArray<T, SIZE, ALIGNMENT> &destination, const AlignedArray<T, SIZE, ALIGNMENT> &source) {
        inline_add_to(destination.ptr()(), source.ptr()(), SIZE);
    }

    template <typename T, size_t ALIGNMENT, class C, class D>
    static void add_to(AlignedData<T, ALIGNMENT, C> &destination, const AlignedData<T, ALIGNMENT, D> &source) {
        inline_add_to(destination.ptr()(), source.ptr()(), ::std::min(destination.size(), source.size()));
    }

    template <typename T, size_t ALIGNMENT, class C, class D>
    static void add_to_frames_up(AlignedData<T, ALIGNMENT, C> &destination, const AlignedData<T, ALIGNMENT, D> &source, size_t framesUp) {
        inline_add_to(
                destination.frame(framesUp),
                source.ptr()(),
                ::std::min(
                        ::std::max(0, static_cast<ssize_t>(destination.size()) - framesUp * AlignedData<T, ALIGNMENT, C>::Metric::alignmentElements),
                        source.size()));
    }
    ...
    template <typename T, size_t ALIGNMENT>
    static void add_to_with_factor(T * destination, const T * source, T factor, size_t size) {
        inline_add_to_with_factor(assumeAligned<ALIGNMENT>(destination), assumeAligned<ALIGNMENT>(source), factor, size);
    }

    template <typename T, size_t SIZE, size_t ALIGNMENT>
    static void add_to_with_factor(AlignedArray<T, SIZE, ALIGNMENT> &destination, const AlignedArray<T, SIZE, ALIGNMENT> &source, T factor) {
        inline_add_to_with_factor(destination.ptr()(), source.ptr()(), factor, SIZE);
    }

    template <typename T, size_t ALIGNMENT, class C, class D>
    static void add_to_with_factor(AlignedData<T, ALIGNMENT, C> &destination, const AlignedData<T, ALIGNMENT, D> &source, T factor) {
        inline_add_to_with_factor(destination.ptr()(), source.ptr()(), ::std::min(destination.size(), factor, source.size()));
    }

    template <typename T, size_t ALIGNMENT, class C, class D>
    static void add_to_with_factor_frames_up(AlignedData<T, ALIGNMENT, C> &destination, const AlignedData<T, ALIGNMENT, D> &source, T factor, size_t framesUp) {
    ...
    static void subtract_from(T * destination, const T * source, size_t size) {
        inline_subtract_from(assumeAligned<ALIGNMENT>(destination), assumeAligned<ALIGNMENT>(source), size);
    }

    template <typename T, size_t SIZE, size_t ALIGNMENT>
    static void subtract_from(AlignedArray<T, SIZE, ALIGNMENT> &destination, const AlignedArray<T, SIZE, ALIGNMENT> &source) {
    ...
    template <typename T, size_t ALIGNMENT, class C, class D>
    static void subtract_from(AlignedData<T, ALIGNMENT, C> &destination, const AlignedData<T, ALIGNMENT, D> &source) {
        inline_subtract_from(destination.ptr()(), source.ptr()(), ::std::min(destination.size(), source.size()));
    }

    template <typename T, size_t ALIGNMENT, class C, class D>
    static void subtract_from_frames_up(AlignedData<T, ALIGNMENT, C> &destination, const AlignedData<T, ALIGNMENT, D> &source, size_t framesUp) {
    ...
    template <typename T, size_t ALIGNMENT>
    static void subtract_from_with_factor(T * destination, const T * source, T factor, size_t size) {
        inline_subtract_from_with_factor(assumeAligned<ALIGNMENT>(destination), assumeAligned<ALIGNMENT>(source), factor, size);
    }

    template <typename T, size_t SIZE, size_t ALIGNMENT>
    static void subtract_from_with_factor(AlignedArray<T, SIZE, ALIGNMENT> &destination, const AlignedArray<T, SIZE, ALIGNMENT> &source, T factor) {
        inline_subtract_from_with_factor(destination.ptr()(), source.ptr()(), factor, SIZE);
    }

    template <typename T, size_t ALIGNMENT, class C, class D>
    static void subtract_from_with_factor(AlignedData<T, ALIGNMENT, C> &destination, const AlignedData<T, ALIGNMENT, D> &source, T factor) {
        inline_subtract_from_with_factor(destination.ptr()(), source.ptr()(), ::std::min(destination.size(), factor, source.size()));
    }

    template <typename T, size_t ALIGNMENT, class C, class D>
    static void subtract_from_with_factor_frames_up(AlignedData<T, ALIGNMENT, C> &destination, const AlignedData<T, ALIGNMENT, D> &source, T factor, size_t framesUp) {
