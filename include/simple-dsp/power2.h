...
namespace simpledsp {
    ...
    namespace base {
        ...
        template<typename SIZE_T, size_t SIZE_OF_SIZE_T>
        struct BaseFillBitsToRight {
        };

        template<class SIZE_T>
        struct BaseFillBitsToRight<SIZE_T, 1> {
        ...
        struct BaseFillBitsToRight<SIZE_T, 2> {
            static inline SIZE_T fill(const SIZE_T x) {
                SIZE_T n = x;
                n = n | (n >> 1);
                n = n | (n >> 2);
        ...
        template<typename SIZE_T>
        struct BaseFillBitsToRight<SIZE_T, 4> {
            static inline SIZE_T fill(const SIZE_T x) {
                SIZE_T n = x;
                n = n | (n >> 1);
                n = n | (n >> 2);
        ...
        template<typename SIZE_T>
        struct BaseFillBitsToRight<SIZE_T, 8> {
            static inline SIZE_T fill(const SIZE_T x) {
                SIZE_T n = x;
                n = n | (n >> 1);
                n = n | (n >> 2);
        ...
        template<typename SIZE_T>
        struct BaseFillBitsToRight<SIZE_T, 16> {
            static inline SIZE_T fill(const SIZE_T x) {
                SIZE_T n = x;
                n = n | (n >> 1);
                n = n | (n >> 2);
        ...
        template<typename SIZE_T>
        class BaseFillBitsToRight<SIZE_T, 0> {
            template<size_t N>
    ...
    struct FillBitsToRight : base::BaseFillBitsToRight<size_t, guaranteedConstexpr ? 0 : sizeof(size_t)> {
        static_assert(std::is_integral<SIZE_T>::value, "Type must be an integral type");

        using base::BaseFillBitsToRight<size_t, guaranteedConstexpr ? 0 : sizeof(size_t)>::fill;
    };

    namespace base {

        template<bool constExpr, typename SIZE_T = size_t>
        class BasePowerOfTwo
        {
