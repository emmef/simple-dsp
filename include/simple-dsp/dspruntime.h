...
namespace simpledsp {
  ...
  public:
    ...
    IOSpec withInputs(size_t inputs) const {
      return { inputs, outputs_ };
    }

    IOSpec withOutputs(size_t outputs) const {
  ...
  protected:
    ...
    void setIOSpec(const IOSpec &ioSpec) noexcept {
      ioSpec_ = ioSpec;
    }
  ...
  public:
     ...
     */
    sdsp_nodiscard const IOSpec &ioSpec() const noexcept {
      return ioSpec_;
    }
  };
  ...
  class SwitchedOwner {
    ...
    T* getTransport() const noexcept { return write_
                                              ? one_
                                              : two_; }
  ...
  public:
    ...
    RuntimeResult init(int tries, T* (*createWithParameters)(Args...), Args... arguments) noexcept {
    ...
    template<typename... Args>
    RuntimeResult writeParameters(int tries, bool (*setParameters)(T*,Args...), Args... arguments) noexcept {
      auto guard = flag_.guard(tries, UseFence::YES);
      if (!guard.isSet()) {
        return RuntimeResult::BUSY;
      }
    ...
    template<typename... Args>
    RuntimeResult checkCompatible(int tries, bool (*check)(T*,Args...), Args... arguments)
    noexcept {
      auto guard = flag_.guard(tries, UseFence::YES);
      if (!guard.isSet()) {
        return RuntimeResult::BUSY;
    ...
    RuntimeResult read(T *&result) noexcept {
      ...
      MemoryFence fence;
      /*
       * Reading is lazy: at most this thread will not see that something has been written.
       */
      if (write_ == read_) {
        result = using_;
        return RuntimeResult::NOOP;
  ...
  class Runtime : public RuntimeInfo<UserConfig> {
    ...
    static bool reconfigureRuntime(
    ...
    static RuntimeConfig *createRuntimeConfig(
            const IOSpec &ioSpec,
            const PhysicalContext &context,
            const UserConfig &userConfig);
    ...
    /**
     * Applies an available new runtime configuration. This method is called from the
     * processCallback and might run in a realtime-context, which puts restrictions on what
     * the method is allowed to do.
     *
     * @return true if the configuration was successfully applied during runtime.
     */
    sdsp_nodiscard virtual RuntimeResult applyRuntimeConfig(RuntimeConfig *) noexcept = 0;
  ...
  public:
     ...
     * @return RuntimeResult::SUCCESS on success, RuntimeResult::INCOMPATIBLE if the IO does not
     * meet the currently configured specification and RuntimeResult::FAILURE otherwise.
     */
...
//    template<class UserConfig, class RuntimeConfig, class IO>
//  class Runtime : public RuntimeInfo<UserConfig> {
//    static_assert(std::is_base_of<IOBase, IO>::value, "IO class does not have IOBase as base.");
//
//    RuntimeConfig config[2];
//
//  protected:
//    /**
//     * Writes a new runtime configuration for changed context and / or configuration.
...
//
//    /**
//     * Returns whether the provided context is runtime-compatible with the current.
//     *
//     * @param context The physical context to test.
//     * @return true if the provided context can be applied at runtime.
//     */
//    sdsp_nodiscard virtual bool canChangeRuntime(const PhysicalContext &context) const noexcept = 0;
//
//    /**
//     * Returns whether the provided configuration is runtime-compatible wit the current.
//     * @param config The configuration to test.
//     * @return true if the provided configuration can be applied at runtime.
//     */
//    sdsp_nodiscard virtual bool canChangeRuntime(const UserConfig &config) const noexcept = 0;
//
//    /**
//     * Writes a new context to the result context. It is assumed that at least one of context
//     * and configuration is different from the current one and that the checks for runtime
...
//     * @param result Contains the new runtime configuration, only on RuntimeResult::SUCCESS.
//     * @return The result of the operation, which is RuntimeResult::NOOP if nothing has changed,
//     * RuntimeResult::INCOMPATIBLE if the changes are incompatible, RuntimeResult::SUCCESS if a
...
//     * @return true if the combination of parameters is valid and a call to reconfigure or the
