...
namespace simpledsp {
  ...
  namespace process_traits {
    ...
    /**
     * Runtime data that a processor uses in a processor callback. This data should be
     * initialised before it is used by the processor. After that, only the processor should
     * reconfigure the runtime data, using a RuntimeConfig instance. This should already be based
     * on useful information like sample-rate and reconfiguring MUST be fast, cannot block,
     * allocate memory or do anything you should not do in a real-time process.
     *
     * @tparam RuntimeConfig The configuration that is used for initialisation and reconfiguring.
     * @tparam Implementation The runtime data implemdetation class, configured with a
     * RuntimeConfig.
     */
    template<class RuntimeConfig, class Implementation>
    class RuntimeDataTraits {
    ...
    public:
      ...
      sdsp_nodiscard bool initConfig(const RuntimeConfig &config) {
        ...
        bool result = reinterpret_cast<Implementation *>(this)->initConfigTrait(config);
       ...
       *
       * This can and should be called from the processing context. If this is a real-time
       * context, the implementation cannot block, allocate and throw.
       *
      ...
      sdsp_nodiscard bool changeConfig(const RuntimeConfig &config) noexcept {
        if (state_ == State::INITIALIZED) {
          return reinterpret_cast<Implementation *>(this)->changeConfigTrait(config);
        }
        return false;
      }
    ...
    class RuntimeConfigTraits {
      sdsp_nodiscard sdsp_force_inline bool operator==(
              const RuntimeConfigTraits &other) const noexcept {
  ...
  class ProcessingNode {
    static_assert(std::is_base_of<
                    process_traits::RuntimeDataTraits<RuntimeConfig, RuntimeData>, RuntimeData>::value,
            "Type RuntimeData should implement process_traits::RuntimeDataTraits.");
    static_assert(std::is_base_of<
            process_traits::RuntimeConfigTraits<RuntimeConfig>, RuntimeConfig>::value,
            "Type RuntimeConfig should implement process_traits::RuntimeConfigTraits");


    typedef process_traits::RuntimeDataTraits<RuntimeConfig, RuntimeData> * RuntimePtr;

    enum class CommandType { RECONFIGURE, STOP };
    enum class RuntimeState {
      NONE, READY, STOPPING, STOPPED
    };
    ...
    public:
      RTGuard(
              QueueProducerConsumer<RuntimePtr> &queue,
              RuntimePtr runtime) :
    ...
    RuntimeState acknowledgeStoppingGetState() {
       ...
       * The change is only effective FROM the state STOPPING. The STOPPING state is only set,
       * in a consistent context, by the stop() method.
       */
  ...
  public:
    ...
    ProcessingNode(size_t runtimes = 1, size_t configs = 2) :
        ...
        runtimeOut_(std::max(runtimes, 1LU)),
        configs_(std::max(configs, 1LU)) {
    ...
    Result setRuntime(const RuntimeConfig &config, RuntimeConstructorArguments... arguments) {
    ...
    Result stop(long waitmillis = 100, int attempts = 100) {
      ...
      if (waitmillis > 0) {
        ...
        for (int attempt = 0; state_ != RuntimeState::STOPPED && attempt < maxAttempts; attempt++) {
