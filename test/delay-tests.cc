...
namespace {
    ...
    using ReadFirstDelay = simpledsp::DelayBasics<simpledsp::DelayAccessType::READ_THEN_WRITE>;
    using WriteFirstDelay = simpledsp::DelayBasics<simpledsp::DelayAccessType::WRITE_THEN_READ>;
                ...
                if (time >= delay) {
                    if (delayed != int(time - delay)) {
                        if (failures == 0) {
                            expected = time - delay;
                            actual = delayed;
                            std::cout << "FAIL time=" << time << "; write=" << writePtr << "; read=" << readPtr << "; buffer:";
    ...
    protected:
        ...
        int access (int newValue) override {
            return ReadFirstDelay::access(act_values + writePtr, act_values + readPtr, newValue);
        }
    };
    ...
    protected:
        ...
        int access (int newValue) override {
            return WriteFirstDelay::access(act_values + writePtr, act_values + readPtr, newValue);
        }
    };
    ...
    struct DelayTestParameters {
        ...
        std::ostream &print(std::ostream &out) const {
            out << (type == simpledsp::DelayAccessType::READ_THEN_WRITE ? "ReadFirstDelay" : "WriteFirstDelay");
            out << "(";
            out << (config == Configuration::READ_THEN_DELTA_WRITE ? "read=" : "write=");
            out << firstValue;
            out << "; delay=";
            out << delay;
            out << ")";
    ...
    public:
        TestCaseGenerator() {
            for (size_t start = 0; start < metric.getSize(); start++) {
                for (Configuration config : { Configuration::READ_THEN_DELTA_WRITE, Configuration::WRITE_THEN_DELTA_READ}) {
                    for (size_t delay = 1; delay <= metric.getSize(); delay++) {
                        parameters.push_back({start, delay, simpledsp::DelayAccessType::READ_THEN_WRITE, config });
                    }
                    for (size_t delay = 0; delay < metric.getSize(); delay++) {
                        parameters.push_back({start, delay, simpledsp::DelayAccessType::WRITE_THEN_READ, config });
                    }
                }
            }
        }
...
// TODO: need many more tests, and useful ones at that...
    ...
    {
        size_t r;
        BOOST_CHECK_THROW(r = ReadFirstDelay::getValidDelay(metric, size_t(0)), std::invalid_argument);
    }
    ...
    {
        BOOST_CHECK_EQUAL(ReadFirstDelay::getValidDelay(metric, metric.getSize()), metric.getSize());
    }

    BOOST_AUTO_TEST_CASE(testWriteFirsMaxDelayIsInvalid)
    {
        size_t r;
        BOOST_CHECK_THROW(r = WriteFirstDelay::getValidDelay(metric, metric.getSize()), std::invalid_argument);
    }

    BOOST_DATA_TEST_CASE(sample, TEST_GENERATOR.testSamples())
    {
        sample.test();
    }
