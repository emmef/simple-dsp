        ...
        if (time >= delay) {
          if (delayed != int(time - delay)) {
            if (failures == 0) {
              expected = time - delay;
              actual = delayed;
              std::cout << "FAIL time=" << time << "; write=" << writePtr << "; read=" << readPtr
                        << "; buffer:";
  ...
  struct DelayTestParameters {
    ...
    std::ostream& print(std::ostream& out) const {
      out << (type == simpledsp::DelayAccessType::READ_THEN_WRITE
              ...
              : "WriteFirstDelay");
      out << "(";
      out << (config == Configuration::READ_THEN_DELTA_WRITE
              ? "read="
              : "write=");
      out << firstValue;
      out << "; delay=";
...
BOOST_AUTO_TEST_SUITE(DelayTests)
  ...
  BOOST_AUTO_TEST_CASE(testWriteFirsMaxDelayIsInvalid) {
    size_t r;
    BOOST_CHECK_THROW(r = WriteFirstDelay::getValidDelay(metric, metric.getSize()),
