...
namespace {
bool operator==(const Step &step1, const Step &step2) {
  return step1.filter() == step2.filter() && step1.input() == step2.input() &&
         step1.lowOut() == step2.lowOut() && step1.highOut() == step2.highOut();
}

std::ostream &operator<<(std::ostream &output, const Step &step) {
  output << "(on " << step.input() << " apply " << step.filter() << " lowOut->"
         << step.lowOut() << "; highOut->" << step.highOut() << ")";
  return output;
}
...
void testSameSteps(const std::vector<Step> &expected,
                   const std::vector<Step> &actual) {
  if (expected.size() != actual.size()) {
    BOOST_CHECK_MESSAGE(false, "Size of crossover plans not the same");
    std::cout << "\t\tSize: expected=" << expected.size()
...
BOOST_AUTO_TEST_CASE(testSameStepsAreEqual) {
  /*
   * This actually tests the equal operator that is necessary for further
   * testing
   */
  Step step1(1, 2, 3, 4);
  Step step2(1, 2, 3, 4);

  BOOST_CHECK_MESSAGE(step1 == step2,
...
BOOST_AUTO_TEST_CASE(testDifferentStepsAreNotEqualFactor) {
  ...
  for (int i = 1; i <= 4; i++) {
    ...
    switch (i) {
    case 1:
      step2 = {step1.filter() * 7, step1.input(), step1.lowOut(),
               step1.highOut()};
      break;
    case 2:
      step2 = {step1.filter(), step1.input() * 7, step1.lowOut(),
               step1.highOut()};
      break;
    case 3:
      step2 = {step1.filter(), step1.input(), step1.lowOut() * 7,
               step1.highOut()};
      break;
    case 4:
      step2 = {step1.filter(), step1.input(), step1.lowOut(),
               step1.highOut() * 7};
      break;
    default:
